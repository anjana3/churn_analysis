...
company_df = df.groupby(['customer_num']).agg({'order_type': {'most_common_order_type' : lambda x:x.value_counts().index[0]},
                                                'date_shipped': {'average_days_between_orders': lambda x: pd.Series(x.unique()).sort_values().diff().mean(),
                                                'median_days_between_orders': lambda x: pd.Series(x.unique()).sort_values().diff().median(),
                                                'max_gap_between_orders': lambda x: pd.Series(x.unique()).sort_values().diff().max(),
                                                'first_order_date': np.min, 'last_order_date': np.max},
...
company_df['company_lifetime_in_days'] = company_df.date_shipped['last_order_date'] - company_df.date_shipped['first_order_date']
...
company_df['orders_per_week'] = (company_df.order_num['num_orders']/company_df['company_lifetime_in_days'].dt.days) * 7

# creating a churn column based on max_gap_between_orders column
churn_period_in_weeks = 24
company_df['Churned'] = np.where(company_df.date_shipped['max_gap_between_orders'].dt.days > (churn_period_in_weeks * 7), 1, 0)
